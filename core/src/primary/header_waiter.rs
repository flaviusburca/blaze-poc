...
pub struct HeaderWaiter {
    ...
    pending: HashMap<Hash, (Round, Sender<()>)>,
}
    ...
    async fn waiter(
        ...
        tokio::select! {
            result = try_join_all(waiting) => {
                result.map(|_| Some(deliver)).map_err(|e| DagError::StoreError(e.to_string()))
            }
