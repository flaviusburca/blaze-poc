...
async fn process_header() {
    ...
    let expected = Vote::new(&header(), &keypair).await;
    ...
    let stored = store
        ...
        .await
        .unwrap()
        .map(|x| bincode::deserialize::<Header>(&x).unwrap());
    assert_eq!(stored.map(|h| h.id), Some(header().id));
}
...
async fn process_header_missing_parent() {
    ...
    // Send a header to the core.
    let header = Header {
        parents: [Hash::default()].iter().cloned().collect(),
        ..header()
    };
...
async fn process_header_missing_payload() {
    ...
    // Send a header to the core.
    let header = Header {
        payload: [(Hash::default(), 0)].iter().cloned().collect(),
        ..header()
    };
    let id = header.id.clone();
    tx_primary_messages
...
async fn process_certificates() {
    mundis_logger::setup_with_default("debug");
    let keypair = keys().pop().unwrap();
    ...
    for x in &certificates {
        let stored = store.read(x.hash().to_vec()).await.unwrap();
